...
export const mousePosition = () => {
    ...
    const [trail, setTrail] = useTrail(5, () => ({ position, config: { tension: 1000, friction: 65 } }))
    useEffect(() => {
...
export const WormyWorm = () => {
    ...
    return (
        <MrWorm>
            ...
            {trail.map((props, i) => <animated.div key={i} style={{ transform: props.position.interpolate(trans), background: Object.values(theme.worm)[i], zIndex: -i }} />)}
        </MrWorm>
    )
}
